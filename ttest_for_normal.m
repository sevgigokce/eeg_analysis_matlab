...
%% sonrasında anova(2 örnek ttest) algoritması ile anlamlı olup olmadıklrına baktık.
%%

% load our data
istatisic = load('normal_matrix.mat');
ds = istatisic.normal_matrix;
...
for i = 1:length(ds)
    % -Inf değerlerini kontrol et ve sıfır ile değiştir
    ds{1, i}(isinf(ds{1, i}(:,1)), 1) = 0;
    ds{1, i}(isinf(ds{1, i}(:,2)), 2) = 0;
    
    % T-test yap
    [~, p] = ttest(ds{1, i}(:,1), ds{1, i}(:,2));
...
% results = [];
% multcomparison_results = [];
% anlamli_degil = [];
% anlamli_degil_index = 1;
% anlamli = [];
% anlamli_index = 1;
% y_duzey_anlamli = [];
% y_duzey_anlamli_index = 1;
% c_d_anlamli= [];
% c_d_anlamli_index = 1;
% 
% 
...
%         % do ttest
% 
%         for j = 1:length(ds{1, i})
%             % İlk sütun için -Inf kontrolü
%             if isinf(ds{1, i}(j, 1))
%                 ds{1, i}(j, 1) = 0;
%             end
% 
%             % İkinci sütun için -Inf kontrolü
%             if isinf(ds{1, i}(j, 2))
%                 ds{1, i}(j, 2) = 0;
%             end
%         end
%         [h,p] = ttest(ds{1, i}(:,1),ds{1, i}(:,2));
...
% 
%     end
%     if results{1,i} > 0.01 && results{1,i}{2,6} < 0.05
...
%     end
% 
%     if results{1,i} > 0.001 && results{1,i}{2,6} < 0.01
